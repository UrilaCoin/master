...
#if defined(NDEBUG)
#error "Urila cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < 200000000 && nHeight > 0)
            return 200 * COIN;
    }
	...
	else if (nHeight <= 2500 && nHeight >= 1001) {
        nSubsidy = 10 * COIN;
    }
	else if (nHeight <= 25000 && nHeight >= 2501) {
        nSubsidy = 35 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	else if (nHeight <= 2500 && nHeight >= 1000) {
        ret = blockValue * 0.2;
    }
	else if (nHeight <= 25000 && nHeight >= 2501) {
        ret = blockValue * 0.7;
